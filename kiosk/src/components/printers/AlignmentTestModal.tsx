/**
 * Ported from portal/src/components/printers/AlignmentTestModal.tsx —
 * same ZPL, same flow, same copy. Two adjustments for the kiosk: the
 * helpers come from @portal (`lib/labels`, `lib/printLabels` are
 * React-free and shared, so the offsets land in the SAME
 * `labels.print.settings` store the portal's Print Labels reads), and
 * the portal's ComboBox — a .tsx the kiosk cannot import without
 * bundling a second React — becomes a native <select> styled by the
 * kiosk's `.kiosk-shell .pf-form select` rule.
 *
...
import { dpiDots, sizeMeta, vocabOfKind } from '@portal/lib/labels';
import {
  alignmentTestZpl, applyPrintSettings, clampSetting, readPrintSettings, writePrintSettings,
} from '@portal/lib/printLabels';
          ...
          <div className="zp-two-col">
            <section className="zp-col" aria-label="Label and offsets">
              ...
              <div className="pf-form zp-form">
                <div>
                  <label htmlFor="at-size">Label size</label>
                  <select id="at-size" aria-label="Label size" value={size?.key ?? ''} onChange={(e) => setSizeKey(e.target.value)}>
