...
export interface KioskFeature {
  id: 'setup' | 'scan' | 'enroll' | 'labels' | 'timeclock' | 'settings';
  path: string;
  title: string;
  blurb: string;
   ...
   *  Kiosk Setup, Scanning, RFID Enroll, Label Printing, and Timeclock —
   *  which is to say all of them today; the flag stays for the next
   *  one). */
  placeholder?: boolean;
