"""import-worker CLI flags: --reload wiring and flag conflicts.
...
def test_help_shows_reload_flag():
    result = runner.invoke(app, ["import-worker", "--help"])
    assert result.exit_code == 0
    assert "--reload" in result.output


def test_reload_and_once_conflict():
    result = runner.invoke(app, ["import-worker", "--reload", "--once"])
...
def test_reload_invokes_watchfiles_run_process(monkeypatch):
    ...
    result = runner.invoke(
        app, ["import-worker", "--reload", "--poll-seconds", "1.5"])
    assert result.exit_code == 0, result.output
    assert calls["target"] is cli._run_worker_process
