...
async def test_list_shows_labels_and_clients(client, db, seeded_user):
    ...
    assert row["type_label"] == "Data centre" and row["type_color"] == "#1668a7"
    assert row["status_label"] == "Planned" and row["status_color"] == "#0f7c86"
...
async def test_lookups_and_survey_schema(client, seeded_user):
    ...
    statuses = (await client.get("/status-values?record_type=site",
                                 headers=hdrs)).json()
    assert statuses[0]["key"] == "active" and statuses[0]["color"] == "#178a4c"
