...
// Every colour a status value can carry. Tokens only — never hex, see
// directory.css for the --c-* custom properties each one resolves to.
export const STATUS_COLOR_TOKENS = [
  'c-green', 'c-amber', 'c-red', 'c-aqua', 'c-slate', 'c-blue', 'c-violet',
] as const;
...
function colorLabel(token: string): string {
  const bare = token.replace(/^c-/, '');
  return bare.charAt(0).toUpperCase() + bare.slice(1);
}

export function ColorSwatch({ token, title }: { token: string; title?: string }) {
  return (
    <span className="color-swatch" style={{ background: `var(--${token}, #8a97aa)` }}
          title={title ?? token} />
  );
}
...
const emptyForm: StatusForm = {
  ...
  color: STATUS_COLOR_TOKENS[0], sort_order: '0', is_active: true,
};
            ...
            <div className="pf-form">
              ...
              <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <ColorSwatch token={form.color} />
                  {/* Native <option> can't render arbitrary markup, so each
                      option's text is tinted with its own colour instead of
                      a true swatch — the live ColorSwatch above covers the
                      current selection. */}
                  <select className="org-select" value={form.color} disabled={locked}
                          onChange={(e) => setField('color', e.target.value)}>
                    {STATUS_COLOR_TOKENS.map((t) => (
                      <option key={t} value={t} style={{ color: `var(--${t})` }}>
                        {colorLabel(t)}
                      </option>
                    ))}
                  </select>
