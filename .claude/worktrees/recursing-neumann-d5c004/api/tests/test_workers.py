...
async def test_levels_seeded_and_staff_readonly(client, seeded_user, db):
    """Editing the level scale is vocabulary and is gated on `devtools`
    (developer-only) — see test_lookup_gates.py for the admin-forbidden /
    developer-allowed matrix. Staff (and admin) stay read-only here."""
    # staff can read
    headers = await _headers(client)
...
async def _dev_headers(client, db, email="lvldev@test.example.com"):
    person = Person(first_name="Dev", last_name="Ops", email=email)
    db.add(person)
    await db.flush()
    db.add(UserAccount(
        person_id=person.id, email=email,
...
async def test_level_explicit_null_on_non_nullable_field_is_422(
        client, seeded_user, db):
    """title/description/expected_skills are all NOT NULL on WorkerLevel
    (db/models.py:236-239). Mirrors update_status_value's pre-check —
    update_level used to silently drop these via an `if value is not None`
    in-loop guard instead of rejecting them."""
    headers = await _dev_headers(client, db)
    for field in ("title", "description", "expected_skills"):
        resp = await client.patch("/worker-levels/L1", headers=headers,
                                  json={field: None})
...
async def test_level_empty_expected_skills_is_accepted_and_persists(
        client, seeded_user, db):
    """The regression guard: expected_skills=[] is a legitimate "no listed
    skills" value, not an absent one. The old `if value is not None` guard
    would have silently dropped it (falsy, but not None — so it actually
    passed that guard!) — this pins that it round-trips for real."""
    headers = await _dev_headers(client, db)
    resp = await client.patch("/worker-levels/L2", headers=headers,
                              json={"expected_skills": []})
    assert resp.status_code == 200
    assert resp.json()["expected_skills"] == []

    levels = (await client.get("/worker-levels", headers=headers)).json()
    l2 = [l for l in levels if l["level"] == "L2"][0]
    assert l2["expected_skills"] == []


async def test_level_empty_patch_writes_no_audit_and_does_not_bump_updated_at(
        client, seeded_user, db):
    from sqlalchemy import select

    from serversherpa.db.models import AuditLog, WorkerLevel

    headers = await _dev_headers(client, db)
    before = await db.get(WorkerLevel, "L3")
    before_updated_at = before.updated_at

    resp = await client.patch("/worker-levels/L3", headers=headers, json={})
    assert resp.status_code == 200

    await db.refresh(before)
    assert before.updated_at == before_updated_at
    audit_row = await db.scalar(select(AuditLog).where(
        AuditLog.entity_type == "worker_level", AuditLog.entity_id == "L3"))
    assert audit_row is None


async def test_all_seeded_worker_statuses_are_assignable(client, seeded_user, db):
    """The three worker rows seeded by 0012_status_values (active, standby,
    blacklist) must all remain assignable now that WorkerProfileIn.status is
    runtime-validated against status_values instead of a frozen Literal."""
    worker = await _mk_worker(db)
    headers = await _headers(client)
    ...
    resp = await client.put(f"/workers/{worker.id}/profile", headers=headers,
                            json={"status": "blacklist", "status_note": "reason"})
    assert resp.status_code == 204
...
async def test_list_workers_carries_status_label_and_color(client, seeded_user, db):
    """The portal renders the chip straight from these, mirroring sites.py:76 —
    so an unlabelled worker row means a blank chip, not a fallback."""
    worker = await _mk_worker(db)
    headers = await _headers(client)

    w = (await client.get("/workers", headers=headers)).json()[0]
    assert (w["status"], w["status_label"], w["status_color"]) == (
        "active", "Active", "c-green")          # profile-less → defaults

    await client.put(f"/workers/{worker.id}/profile", headers=headers,
                     json={"status": "standby"})
    w = (await client.get("/workers", headers=headers)).json()[0]
    assert (w["status_label"], w["status_color"]) == ("Standby", "c-amber")


async def test_worker_on_deactivated_status_keeps_its_real_label(
        client, seeded_user, db):
    """status_labels() must NOT filter is_active. Retirement is is_active (there
    is no DELETE), and worker_profiles.status has a composite FK into
    status_values — so a worker CAN sit on a deactivated status, and it must
    still render as itself rather than degrading to the raw key."""
    worker = await _mk_worker(db)
    headers = await _headers(client)
    await client.put(f"/workers/{worker.id}/profile", headers=headers,
                     json={"status": "standby"})
    ...
    await db.execute(text(
        "UPDATE status_values SET is_active = false "
        "WHERE record_type = 'worker' AND key = 'standby'"))
    await db.commit()
