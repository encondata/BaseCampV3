...
class SiteLookupUpdateIn(BaseModel):
    # Fields mirror update_site_type's mutable set exactly. `color` used to
    # live here while this schema was shared with the /site-statuses PATCH,
    # but 0012_status_values folded site_statuses into status_values and that
    # route went with it — colour is now edited via /status-values. Adding a
    # field back that the handler doesn't write is how you get a silent no-op.
    label: str | None = None
    description: str | None = None
    sort_order: int | None = None
    icon: str | None = None
