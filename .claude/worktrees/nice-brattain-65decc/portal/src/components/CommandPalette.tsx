...
export default function CommandPalette() {
  ...
  const { can, preferences, updatePreferences } = useAuth();
  ...
  const commands = useMemo<Command[]>(() => {
    ...
    const navGated = (label: string, to: string, resource: string): Command[] =>
      can(resource, 'view') ? [nav(label, to)] : [];
    const cmds: Command[] = [
    ...
    return cmds;
  }, [users, navigate, preferences, updatePreferences, can]);
