...
**Enroll gates (Android only, 2026-09-16).** The web kiosk warns that a new tag replaces an old one and otherwise lets the save through. The Android app refuses the two mistakes a busy floor makes instead of reporting them afterwards, in `core/scan/EnrollGate.kt`:

- An asset that already carries a tag stops at a confirmation step. Its details and its current tag are on screen with an **Update RFID Value** button and Cancel; the tag box does not exist until the operator confirms, so a stray read from a neighboring reader cannot retag anything. A scan arriving at that step is refused with "{name} already has a tag. Tap Update RFID Value to replace it."
- An asset this kiosk enrolled since the screen opened comes back to the same step, worded "You just enrolled {name}…", with the tag from the session log rather than the roster (the log is the fresher of the two and survives a failed local roster update).
- Before the POST, `checkEnrollTag` refuses a tag that is already on this asset ("That tag is already on this asset."), one this session handed to another asset, or one the synced roster gives to another asset. Comparison is on `rfidKey` (zero-padding stripped, upper-cased), so a padding handheld and a non-padding reader cannot slip a repeat past it. The check runs again inside the send coroutine, because a sync may have rebuilt the roster while the operator lined the reader up. The portal's own `rfid_in_use` remains the last word.

The session log is `EnrollUi.enrollments` (25 rows, memory only) and each row now carries the asset row id it belongs to.
