...
- **Per-scan rejection, never per-batch.** A scan naming a site, move, or checkpoint the server does not know is rejected on its own with `bad_site` / `bad_initiative` / `bad_status` and the rest of the batch still lands; the response is 200 with both lists. One stale row must not cost a truckload of scans. A checkpoint only has to *exist* — a status deactivated mid-move keeps ingesting. The whole batch fails only on an unknown or non-kiosk serial (404 `device_not_found`), a malformed body (422), or read-only mode (423).
...
| Restart | `startSender()` resumes `queued`/`retrying` rows on mount; a row left `sending` by a reload resets to `queued` — ingest is idempotent on `client_scan_id`, so re-sending one that landed is free. |
