...
- `GET /kiosk/sync/assets?initiative_id=<uuid>` → `KioskAssetsSyncOut {initiative_id, initiative_name, generated_at, assets: [KioskAssetOut]}`, where `KioskAssetOut = {id, asset_id, name, rfid, serial_number, make, model, make_model, label}`. `asset_id` is `Asset.legacy_id` (the human Asset ID), `rfid` is `Asset.rfid_tag`, make/model come from `asset_models` via `Asset.model_id`. The roster is `initiative_assets` joined to `assets` (+ outer-joined `asset_models`), ordered by `legacy_id`; an asset not on the roster is never returned. `label` is the **label placeholder map** for that asset on that move, built by the label generator's own `labels/generate/values.py::placeholder_values()` over the active `label_placeholders` catalog minus `CONTAINER_KEYS` — i.e. `asset_id`, `asset_name`, `serial_number`, `make`, `model`, `make_model`, `source_raw`, `source_ru`, `source_site`, `destination_raw`, `destination_ru`, `destination_site`, `move_name`, `move_date`. Reusing the resolver (rather than re-deriving the fields here) is what guarantees an offline kiosk renders the same values a generated label carries, RU formatting (`U14`) and all. 404 `initiative_not_found`; 422 `bad_initiative` when the initiative is not a move.
- `GET /kiosk/sync/people` → `KioskPeopleSyncOut {generated_at, people: [{id, display_name, rfid_tag, is_worker, has_account}]}`: every `Person` with `archived_at IS NULL` that has a `worker_profiles` row **or** a `user_accounts` row, ordered by last then first name. A plain contact (neither) is never included.
