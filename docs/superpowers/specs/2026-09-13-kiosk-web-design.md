...
**Deferred**: manual `scan_type` (no keyed-entry mode), camera scanning, scan sounds (they belong to the Sound tab), per-scan overrides of the kiosk's site/move/checkpoint, and any unmatched-scan lookup against the server.
