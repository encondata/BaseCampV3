...
**`pages/KioskSetup.tsx`**: eyebrow "Kiosk · Setup", title "Kiosk setup". With a saved selection and `kiosk_setup_complete === 'complete'`, shows a `.setup-summary` card ("This kiosk is set up for **{name}** at **{siteName}** ({siteRole}) · scan type **{label}**") with "Change setup" (`mini-btn`, re-enters the wizard at step 1 with all three fields pre-selected) and "Go to home" (`btn-solid`, → `/`). Otherwise a `.setup-wizard` with a `.setup-steps` indicator ("Step 1 of 3 · Move" / "Step 2 of 3 · Site" / "Step 3 of 3 · Scan type") and a `.pf-form` with one field per step — native `<select>` elements, not the portal's `ComboBox` (a `.tsx` the kiosk cannot import across the two-Reacts boundary) — and large touch targets matter more here than the portal picker's affordances anyway. Step 1's options load on mount via `getSetupOptions()` ("Loading moves…" while pending; a load failure shows `.form-error` "Couldn't load setup options." with a `mini-btn` Retry; an empty initiatives list shows "No active moves. Ask a coordinator to plan one." and keeps Next disabled). A planned move's option label appends " (planned)"; in-progress moves show no suffix. Changing the move on step 1 clears the site choice. Step 2 ("Which site is this kiosk at?") lists up to two options built from the chosen move's `source_site`/`destination_site` ("{name} — source" / "{name} — destination", skipping a null side); a move with neither site shows "This move has no sites yet. Ask a coordinator to add them." and keeps Next disabled. Finish calls `submitKioskSetup({serial: getIdentity().serial, initiative_id, site_id, scan_status})`; success caches the response names via `writeKioskSetup`, sets `kiosk_setup_complete` to `complete`, and swaps to the summary; failure sets it to `failed` and shows an inline `.form-error` "Couldn't save the kiosk setup ({code}). Try again." without leaving step 3.
