...
**Flash** (`lib/flash.ts`, `components/ScanFlash.tsx`, `lib/appearance.ts`). A match flashes the whole screen `good_scan`; no match flashes `not_found_scan`. Both colors live in `ss.kiosk.appearance` as HSL channels, kiosk-local (the right green for a dark cage is not the right green for an office, and the color belongs to the screen rather than the signed-in account), defaults `good {h:150,s:60,l:45}` / `not-found {h:0,s:70,l:50}`. The Appearance tab edits them with an `HslPicker` each — swatch, three sliders, `hsl(...)` readout, Preview flash — persisting on every drag. The overlay is rendered once in `KioskShell` (fixed, `inset:0`, click-through, above the shell), fades 0.85 → 0 over its duration, and holds a short solid flash instead under `prefers-reduced-motion`.
