...
The first real Kiosk Setup flow: a two-step wizard at `/setup` — pick a move, then a scan type — that stamps the kiosk's `Device` row server-side and marks `kiosk_setup_complete`. This replaces the earlier "Kiosk Setup is a placeholder" state (see "This Kiosk moves off /setup" above): `setup` drops `placeholder: true` in `features.ts` (it stays `alwaysAvailable`), and `/setup` moves from outside `KioskGuard` to inside it — setup writes a Device row on the signed-in person's behalf, so it now requires a signed-in person like every other feature route, while `featureAvailable` keeps it (and Settings) usable regardless of `kiosk_setup_complete`.

**Endpoints** (`api/src/serversherpa/api/routes/kiosk.py`, both `kiosk:view`):
- `GET /kiosk/setup-options` → `SetupOptionsOut {initiatives: [{id, name, status}], scan_types: [{key, label, color}]}`. Initiatives are `initiative_type == 'move'`, `status in (planned, in_progress)`, `archived_at IS NULL`, ordered by name — the exact filter `DeviceEditModal.tsx`/`KioskDevices.tsx` already apply client-side in the portal. Scan types are active `asset` status values, ordered by `sort_order` then label. This endpoint exists because workers hold `kiosk:view` but not `initiatives:view` — they cannot call `/initiatives` directly, so the kiosk gets only the narrow slice it needs.
- `POST /kiosk/setup` body `KioskSetupIn {serial, initiative_id, scan_status}` → `KioskSetupOut {device_id, initiative_id, initiative_name, scan_status, scan_status_label}`. Finds the kiosk `Device` by serial (404 `device_not_found` if absent or not a kiosk), re-validates `initiative_id` against the same move/planned-or-in_progress/not-archived filter (422 `bad_initiative`) and `scan_status` against the active asset-status keys (422 `bad_scan_status`) — never trusts the options list a stale client cached — then sets `current_initiative_id`, `scan_status`, `updated_at`, and audits `action="kiosk_setup"` on `entity_type="device"` with `changes={initiative_id, scan_status}`. Not read-only exempt (it writes); 423 under read-only mode.

**Kiosk selection cache** (`src/lib/kioskSetup.ts`): `KioskSetupSelection {initiativeId, initiativeName, scanStatus, scanLabel}`, kiosk-local, mirrors `devMode.ts`/`setupState.ts`'s store/hook shape (`readKioskSetup`/`writeKioskSetup`/`clearKioskSetup`/`subscribeKioskSetup`/`useKioskSetup`, localStorage key `ss.kiosk.setup`, same try/catch idiom). The server Device row is the source of truth; this is only a local cache of the display names (from the `/kiosk/setup` response) so the summary card and footer render without an extra fetch. `useSyncExternalStore` needs a referentially stable snapshot when nothing changed or it re-renders forever, so `read()` caches the parsed object against the raw string it came from rather than re-parsing every call.

**`pages/KioskSetup.tsx`**: eyebrow "Kiosk · Setup", title "Kiosk setup". With a saved selection and `kiosk_setup_complete === 'complete'`, shows a `.setup-summary` card ("This kiosk is set up for **{name}** · scan type **{label}**") with "Change setup" (`mini-btn`, re-enters the wizard at step 1 with both fields pre-selected) and "Go to home" (`btn-solid`, → `/`). Otherwise a `.setup-wizard` with a `.setup-steps` indicator ("Step 1 of 2 · Move" / "Step 2 of 2 · Scan type") and a `.pf-form` with one field per step — native `<select>` elements, not the portal's `ComboBox` (a `.tsx` the kiosk cannot import across the two-Reacts boundary) — and large touch targets matter more here than the portal picker's affordances anyway. Step 1's options load on mount via `getSetupOptions()` ("Loading moves…" while pending; a load failure shows `.form-error` "Couldn't load setup options." with a `mini-btn` Retry; an empty initiatives list shows "No active moves. Ask a coordinator to plan one." and keeps Next disabled). A planned move's option label appends " (planned)"; in-progress moves show no suffix. Finish calls `submitKioskSetup({serial: getIdentity().serial, initiative_id, scan_status})`; success caches the response names via `writeKioskSetup`, sets `kiosk_setup_complete` to `complete`, and swaps to the summary; failure sets it to `failed` and shows an inline `.form-error` "Couldn't save the kiosk setup ({code}). Try again." without leaving step 2.

**Footer**: `KioskShell` appends `Move`/`Scan` items after `Setup` whenever a selection is saved (`useKioskSetup()`), showing the cached `initiativeName`/`scanLabel`.

**CSS** (`kiosk.css`): `.setup-steps`, `.setup-wizard` (max-width 560px), `.setup-summary` (card), `.setup-actions` (button row). Also a minimal `.kiosk-shell .pf-form select` rule — the portal's `.pf-form select` styling lives in `notifications.css`, which `main.tsx` never imports (only `base.css`/`portal-theme.css`/`auth-theme.css`/`directory.css`/`chrome.css`/`profile.css`), so the kiosk restates a large-touch-target version scoped to `.kiosk-shell` rather than pull in a stylesheet built for a different page.
