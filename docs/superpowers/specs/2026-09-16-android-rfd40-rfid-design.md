...
- The "SDK not present at runtime" row in the Errors table above promises the RFID tab will say the reader library is missing. That is only true of `connect()`: its `LinkageError` catch routes through the `readable()` helper and surfaces the same generic "Couldn't connect to the reader (...)" sentence as any other connect failure. Its sibling catches in `apply()`/`startInventory()`/`stopInventory()` don't go through `readable()` at all — each just returns `Result.failure(e)` with the raw `LinkageError`'s own message. For `apply()`, that raw message reaches the operator via `RfidController.push()`'s `_applyError` reporting, shown on the RFID tab as "The reader refused a setting: `<raw message>`" — not the connect-failure sentence. For `startInventory()`/`stopInventory()`, `RfidController` doesn't surface the failure at all: `onTrigger`'s `TriggerAction.START` branch and `endBurst` both swallow it silently and carry on as if the call had succeeded, so a `LinkageError` there reaches the operator as nothing — not even a sentence. Distinguishing "the library itself failed to load" from an ordinary vendor failure, uniformly across all four call sites, would need a dedicated check this pass didn't add.
