...
from serversherpa.db.models import AuditLog, AuthSession, UserAccount
...
async def test_kiosk_login_refused_without_kiosk_permission(client, db, seeded_user):
    await _make(db, client, "client_viewer", "cv@test.example.com")   # logs in once (portal)
    ...
    assert len(sessions) == 1                      # only _make's portal login
...
async def test_portal_login_unchanged_for_client_viewer(client, db, seeded_user):
    await _make(db, client, "client_viewer", "cv2@test.example.com")
    resp = await client.post("/auth/login", json={
...
async def test_kiosk_login_wrong_password_is_still_invalid_credentials(client, db, seeded_user):
    await _make(db, client, "client_viewer", "cv3@test.example.com")
    resp = await client.post("/auth/login", json={
...
async def test_phone_side_requires_kiosk_permission(client, db, seeded_user):
    d = await _create(client)
    cv = await _make(db, client, "client_viewer", "cv@test.example.com")
