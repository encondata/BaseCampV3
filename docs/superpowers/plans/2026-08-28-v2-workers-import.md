...
- Every imported worker gets a `PersonRole(role="worker")` grant (granted_by NULL = bootstrap convention) — the portal's Workers page lists people by that grant, not by WorkerProfile existence (found during live verification; fixed in `fix(api): imported V2 workers get the worker role grant`).
