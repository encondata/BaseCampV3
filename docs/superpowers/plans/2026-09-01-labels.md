...
- Produces: `GET /labels/vocab[?kind=]` → `list[LabelVocabOut]` (gate `labels:view`; includes `usage_count`); `POST /labels/vocab` (gate `devtools:add`, 409 `label_vocab_exists`, 422 `bad_kind`/`bad_meta`); `PATCH /labels/vocab/{kind}/{key}` (gate `devtools:change`, 404 `unknown_vocab`); router object `labels.router`; schemas `LabelVocabOut`, `LabelVocabCreateIn`, `LabelVocabUpdateIn`.
