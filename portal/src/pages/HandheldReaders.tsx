/** Handheld Readers — the device-fleet directory list for Android, iOS,
 *  and Zebra handheld scanners. Standalone page (own .portal-page/.dir-
 *  head, model: Notifications.tsx) built on the shared directory-list
 *  pattern (model: components/statusRules/RulesTab.tsx — the freshest
 *  full-pattern list): search + toolbar FilterButton facet
 *  (Type/Registration/Site) + per-column ColumnMenu filters + persisted
 *  visible/sort/order state (usePersistentListState) + CSV export +
 *  virtualized rows.
 *
 *  Copy of pages/KioskDevices.tsx — same directory-list/create/register
 *  lifecycle, adapted for handheld_reader devices: "+ New handheld" opens
 *  the shared DeviceEditModal (handheld deviceType/noun/typeOptions) in
 *  create mode; row Actions → Edit opens it prefilled; Register/Renew
 *  opens RegisterDaysModal → registerDevice; De-Register confirms then
...
export default function HandheldReaders() {
  const { can } = useAuth();
  ...
  const {
    ...
    'hardware-handhelds', { visible: DEFAULT_VISIBLE, sortKey: 'name', sortDir: 1 }, ALL_COLUMN_KEYS,
  );

  const load = async () => {
    try {
      setDevices(await listDevices('handheld_reader'));
      setError('');
    } catch (err) {
      setError(err instanceof ApiError && err.status === 403
        ? "You don't have access to scanning hardware."
        : "Couldn't load handheld readers.");
    }
  };
    ...
    <div className="portal-page">
      <div className="dir-head">
        <div>
          ...
          <p className="page-hint">Android, iOS, and Zebra handheld scanners.</p>
        </div>
      </div>
      ...
      <div className="dir-toolbar">
        <div className="toolbar-right">
          ...
          <ExportButton onExport={() => exportCsv('handheld-readers', CSV_COLUMNS, visible)} />
          {canAdd && (
      ...
      {error && (
        <div className="dir-empty" style={{ marginBottom: 12 }}>
          <b>{devices ? "Couldn't complete that action" : 'Cannot load handheld readers'}</b>{error}
        </div>
      )}
          ...
          {visible.length === 0 && (
            devices.length === 0 ? (
              <div className="dir-empty">
                No handheld readers yet — provision the first one.
              </div>
            ) : (
              <div className="dir-empty">
      ...
      {editing !== null && (
        <DeviceEditModal
          deviceType="handheld_reader" noun="handheld"
          typeOptions={[
