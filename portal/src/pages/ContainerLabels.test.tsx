...
it('bulk Set tag PATCHes only the selected containers, never an unselected one', async () => {
  ...
  const user = userEvent.setup();
  await pickInitiativeAndContainers(user);                 // selects c1
  await user.click(screen.getByText('Server Bin'));        // also select c2 — c3 stays unselected
...
it('a concurrent second tag change is not undone when an earlier one\'s PATCH later rejects', async () => {
  ...
  // c1's tag change starts — its PATCH is left pending (in flight).
  await user.click(screen.getByLabelText('Tag for Rack Cart 1'));
  await user.click(await screen.findByRole('menuitem', { name: /Priority/ }));
  expect(screen.getByLabelText('Tag for Rack Cart 1').textContent).toContain('Priority'); // optimistic

  // c2's tag change starts (a different container) and resolves normally
  // while c1's PATCH is still in flight.
  ...
  // Now c1's PATCH rejects — only c1 should revert.
