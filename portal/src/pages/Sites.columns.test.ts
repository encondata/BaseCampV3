/** Sites has no test file to carry a rendered column-floors assertion
 *  (see recipe R7) — this asserts the pure computation instead. */
import { describe, expect, it } from 'vitest';

import { listGridStyle } from '../lib/listTools';
import { PRIMARY_COL, SITE_COLUMNS } from '../lib/sites';

describe('Sites columns', () => {
  it('default columns fit .portal-page at a 1512px window, nav expanded', () => {
    const defaults = [PRIMARY_COL, ...SITE_COLUMNS.filter((c) => c.default)];
    expect(listGridStyle(defaults, ['30px']).minWidth).toBeLessThanOrEqual(1176);
  });
});
