 ...
 * an update's diff with its Update box, and the Skip box. Pure rendering;
...
export default function TeamBulkRowDetails({
  row, options, picked, skipped, approved, disabled,
  onPick, onClearPicks, onToggleSkip, onToggleApprove,
}: Props) {
  const n = row.row;
    ...
    <div className="bulk-diff">
      ...
      {canSkip && (
        <label>
          ...
          {' '}{skipped ? 'Skipped — untick to undo' : 'Skip'}
        </label>
      )}
